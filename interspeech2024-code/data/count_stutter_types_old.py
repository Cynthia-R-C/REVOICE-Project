# Cynthia Chen 7/11/2026
# Counts how many valid audio clips per stutter type exist in the raw_data (old) SEP-28k+FluencyBank dataset
...
# Defaults match running this from ScienceFair2025/interspeech2024-code/data, per
# your directory layout - override with --labels/--clips if you run it elsewhere.
DEFAULT_LABELS = ["../../raw_data/SEP-28k/SEP-28k_labels.csv"]
DEFAULT_CLIPS = "../../raw_data/SEP-28k/clips/stuttering-clips/clips"
...
def main():
    parser = argparse.ArgumentParser(description="Count stutter-type labels for clips that actually exist on disk (old dataset, flat naming).")
    parser.add_argument("--labels", nargs="+", default=DEFAULT_LABELS,
                         help=f"One or more label CSV files (default: {DEFAULT_LABELS})")
    parser.add_argument("--clips", type=str, default=DEFAULT_CLIPS,
                         help=f"Path to the flat clips directory (default: {DEFAULT_CLIPS})")
    args = parser.parse_args()

    clips_dir = pathlib.Path(args.clips)
    if not clips_dir.exists():
        print(f"ERROR: clips directory not found: {clips_dir.resolve()}")
        print("Check you're running this from ScienceFair2025/interspeech2024-code/data,")
        print("or pass --clips with the correct path for wherever you're running it from.")
        return

    frames = []
    for path in args.labels:
        if not pathlib.Path(path).exists():
            print(f"ERROR: labels file not found: {pathlib.Path(path).resolve()}")
            return
        # dtype={"EpId": str} matches extract_clips.py / the Kaldi file_ID convention
    ...
    print(f"Checking clips in: {clips_dir.resolve()}\n")
    ...
    # OLD dataset naming (flat, per your Kaldi file-prep docs):
    # file_ID = Show_EpId_ClipId, wav_dir = clip_path / f"{file_ID}.wav"
    def clip_exists(row):
        show = str(row[show_col])
        ep = str(row[ep_col]).strip()
        clip_id = row[clip_col]
        clip_path = clips_dir / f"{show}_{ep}_{clip_id}.wav"
        if clip_path.exists() and not is_audio_valid(clip_path):

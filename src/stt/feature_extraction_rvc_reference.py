...
if __name__ == "__main__":
    ...
    # Method for starting voice conversion
        ...
        # Purpose: initialize live audio capture and playback using low-latency settings if needed to feed data into the Harvest-based conversion pipeline
        def start_stream(self):
            global flag_vc

            # Turn on stream-on indicator
            if not flag_vc:
                flag_vc = True

                # For WASAPI with exclusive mode activated, add extra settings
                if (
                ...
                # otherwise no extra settings
                else:
                    extra_settings = None

                # Create a sounddevice.Stream and starts it
        ...
        # Stops the audio stream safely to halt realtime audio processing when the user stops the conversion or changes settings
        ...
        def audio_callback(
            ...
            # Silences quiet audio segments ot reduce noise; purpose is to gate low-volune input based on the user's threshold, improving clarity before Harvest pitch processing
